...
export default function PlayerCard({ player, team }: PlayerCardProps) {
  ...
  return (
    ...
    >
      {/* Glass card */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-white/[0.06] to-white/[0.02] backdrop-blur-xl border border-white/[0.08] p-5 transition-all duration-300 hover:border-white/[0.15] hover:bg-white/[0.08] hover:shadow-lg hover:shadow-black/20 hover:-translate-y-1">
        ...
        <div className="relative flex flex-col items-center text-center">
          ...
          <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-3 ${getPositionColor(player.position)}`}>
            #{player.number} • {positionShort[player.position]}
          </div>

          {/* Name */}
          <h3 className="font-semibold text-white/95 text-sm mb-1 line-clamp-1 group-hover:text-white transition-colors">
            {player.name}
          </h3>
          ...
          {(player.tournamentGoals > 0 || player.tournamentAssists > 0) && (
            <div className="flex items-center justify-center gap-3 mt-3 px-3 py-1.5 rounded-lg bg-white/[0.03]">
