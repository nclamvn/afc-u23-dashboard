...
import { getPlayerById, chinaPlayers } from '@/data/players';
...
export default async function ChinaPlayerPage({ params }: PageProps) {
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="relative">
            ...
            {player.isStar && !player.isCaptain && (
              <div className="absolute top-4 right-4 bg-gradient-to-br from-[#ff375f] to-[#dc143c] text-white rounded-xl p-2 shadow-lg">
                <Sparkles size={20} />
              </div>
            )}
          </div>
            ...
            <div className="flex items-start justify-between mb-6">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-2xl font-bold text-[#ff375f]">#{player.number}</span>
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-[#de2910]/20 to-[#ffde00]/20 flex items-center justify-center border border-white/10">
              <span className="text-4xl">{player.flag}</span>
            </div>
            <span className="text-sm text-white/60">CHINA U23</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <StatCard title="Goals in Tournament">
          ...
          <div className="mt-4 flex justify-around text-center">
            <div>
              <div className="text-xl font-bold text-[#ff375f]">{player.tournamentGoals}</div>
      ...
      <StatCard title="AFC U23 Asian Cup 2026 Best Scorers" actions={{ external: true }}>
        <TopScorersTable scorers={topScorers.slice(0, 6)} highlightTeam="China" />
      </StatCard>
    </DashboardLayout>
  );
}
...
export async function generateStaticParams() {
  return chinaPlayers.map((player) => ({
    playerId: player.id,
  }));
}
